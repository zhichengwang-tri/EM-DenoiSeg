...
def loop(cfg, train_provider, valid_provider, model_dn,model_seg, ema_model, optimizer_dn,iters, writer):
    iters = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='dms_c_v2', help='path to config file')
    ...
    if args.mode == 'train':
        ...
        model_seg,_ = resume_params(cfg,model_seg,True)
        loop(cfg, train_provider, valid_provider, model_denoise , model_seg, ema_model, optimizer_dn, 0 , writer)
        writer.close()
    else:
        pass
    print('***Done***')