...
def build_2model(cfg, writer):
    ...
    model_denoise = UNet2D(in_channel=1, out_channel=10, if_sigmoid=True, type = 'denoise',fa=False).to(device)
    model_seg = UNet2D(in_channel=1, out_channel=10, if_sigmoid=True, type = 'seg-cons').to(device)
...
def resume_params(cfg, model_dn, model_seg, resume):
    if resume:
        t1 = time.time()
        ...
        state_dict = checkpoint['modeldn_weights']
        partial_dict = {}
        for k,v in state_dict.items():
            partial_dict[k] = v
        print('Resuming weights from %s ... ' % model_path, end='', flush=True)
        if os.path.isfile(model_path):
            model_dn.load_state_dict(partial_dict,strict = False)
        else:
            raise AttributeError('No checkpoint found at %s' % model_path)
        
        state_dict = checkpoint['modelseg_weights']
        partial_dict = {}
        for k,v in state_dict.items():
            partial_dict[k] = v
        print('Resuming weights from %s ... ' % model_path, end='', flush=True)
        if os.path.isfile(model_path):
            model_seg.load_state_dict(partial_dict,strict = False)
        else:
            raise AttributeError('No checkpoint found at %s' % model_path)
        print('Done (time: %.2fs)' % (time.time() - t1))
        return model_dn, model_seg, checkpoint['current_iter']
    else:
        return model_dn, model_seg, 0
...
def loop(cfg, valid_provider,model_dn, model_seg,  writer):
    ...
    for k, batch in tqdm(enumerate(dataloader, 0)):
        ...
        with torch.no_grad():
            ...
            _,pred_affs = model_seg(denoised)
        ...
        affinity_to_visualize = pred_affs[0,0,:].cpu().detach().numpy()  # Taking the first channel for example
        affinity_gt =  target_affs[0,0,:].cpu().detach().numpy()
        # Binarize the affinity map
        threshold = 0.5  # This is an arbitrary threshold, adjust it based on your data
        binary_affinity = (affinity_to_visualize > threshold).astype(np.float32)
        binary_affinity_uint8 = (binary_affinity * 255).astype(np.uint8)
        binary_affinity_gt_uint8 = affinity_gt.astype(np.uint8) *255
        # Create image from the numpy array
        binary_image = Image.fromarray(binary_affinity_uint8)
        binary_image_gt = Image.fromarray(binary_affinity_gt_uint8)
        # post-processing
        ...
        os.makedirs(os.path.join(cfg.valid_path, 'affinity'), exist_ok=True)
        os.makedirs(os.path.join(cfg.valid_path, 'affinity_gt'), exist_ok=True)
        '''save picturesc seg'''
        ...
        binary_image.save(os.path.join(cfg.valid_path, 'affinity','{0}.png'.format(k)) )
        binary_image_gt.save(os.path.join(cfg.valid_path, 'affinity_gt','{0}.png'.format(k)) )
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='test_dms_ac4', help='path to config file')
    ...
    if args.mode == 'train':
        ...
        model_dn,model_seg = build_2model(cfg, writer)
        model_dn, model_seg,  init_iters = resume_params(cfg, model_dn, model_seg, cfg.TRAIN.resume)
        loop(cfg, valid_provider, model_dn, model_seg , writer)
        writer.close()
    else:
        pass
    print('***Done***')