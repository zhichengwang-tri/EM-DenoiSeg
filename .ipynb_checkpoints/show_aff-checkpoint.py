...
def build_2model(cfg, writer):
    ...
    model_seg = UNet2D(in_channel=1, out_channel=10, if_sigmoid=True, type = cfg.MODEL_seg.model_type,fa=False).to(device)
    cuda_count = torch.cuda.device_count()
    if cuda_count > 1:
        if cfg.TRAIN.batch_size % cuda_count == 0:
            print('%d GPUs ... ' % cuda_count, end='', flush=True)
            model_denoise = torch.nn.DataParallel(model_denoise,device_ids = [0])
            model_seg = torch.nn.DataParallel(model_seg,device_ids = [0])
        else:
...
def resume_params_seg(cfg, model, resume):
    if resume:
        t1 = time.time()
        ...
        model_path = os.path.join(cfg.TRAIN.resume_path_seg, 'model-%06d.ckpt' % cfg.TRAIN.model_id_seg)
        checkpoint = torch.load(model_path)
        state_dict = checkpoint['model_weights']
        partial_dict = {}
        for k,v in state_dict.items():
...
def loop(cfg, valid_provider, model_seg,  writer):
    ...
    f_valid_txt.write(' VOI=%.6f, ARAND=%.6f' % \
        (mean_voi,mean_arand))
    f_valid_txt.write('\n')
    f_valid_txt.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='test_c_cl', help='path to config file')
    ...
    if args.mode == 'train':
        ...
        model_seg,  init_iters = resume_params_seg(cfg, model_seg, cfg.TRAIN.resume)
        loop(cfg, valid_provider, model_seg , writer)
        writer.close()
    else:
        pass
    print('***Done***')