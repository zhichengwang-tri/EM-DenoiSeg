...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='sequential_ac34', help='path to config file')
    ...
    if args.mode == 'train':
        ...
        if cfg.TRAIN.opt_type == 'sgd':
            ...
            optimizer_seg = torch.optim.Adam(filter(lambda p: p.requires_grad,model_seg.parameters() ), lr=cfg.TRAIN.base_lr, betas=(0.9, 0.999),
                                    eps=0.01, weight_decay=1e-6, amsgrad=True) 
        model_denoise,_ = resume_params(cfg,model_denoise,True)
        loop(cfg, train_provider, valid_provider, model_denoise , model_seg, ema_model, optimizer_seg, 0 , writer)
        writer.close()
    else:
        pass
    print('***Done***')