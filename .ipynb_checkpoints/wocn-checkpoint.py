'''
w/o coarse denoising net as ablation
'''
...
def build_3model(cfg, writer):
    ...
    if cfg['MODEL']['MODEL_uni']['model_type'] == 'unet2d':
...
def resume_params(cfg, model_seg,model_c_denoise,resume):
    if resume:
        t1 = time.time()
        ...
        if os.path.isfile(model_path):
            ...
            model_seg.load_state_dict(checkpoint['modelseg_weights'])
            model_c_denoise.load_state_dict(checkpoint['modeldn_weights'])
        else:
            raise AttributeError('No checkpoint found at %s' % model_path)
        print('Done (time: %.2fs)' % (time.time() - t1))
        print('valid %d' % checkpoint['current_iter'])
        return model_seg, model_c_denoise, checkpoint['current_iter']
    else:
        return model_seg, model_c_denoise,0
            ...
            if iters % cfg.TRAIN.valid_freq == 0 or iters == 1000 and iters>50000:
                ...
                for k, batch in tqdm(enumerate(dataloader, 0)):
                    ...
                    with torch.no_grad():
                        pred_affs = model_seg(inputs)
                        pred_dn = model_denoise(inputs,pred_affs)
                    pred_dn = torch.clamp(pred_dn,-1,1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='wocn_ac34', help='path to config file')
    ...
    if args.mode == 'train':
        ...
        model_seg,model_c_denoise,init_iters = resume_params(cfg, model_seg,model_c_denoise,cfg.TRAIN.resume)
        loop(cfg, train_provider, valid_provider, model_seg,model_c_denoise, model_denoise, ema_model, optimizer_seg,optimizer_c_denoise,optimizer_denoise, 0, writer)
        writer.close()
    else:
        pass
    print('***Done***')