...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        model_path = '/braindat/lab/wangzc/code/jdas/pretrained_model/vit_checkpoint/imagenet21k/resnet50.pth'
...
class UNET(nn.Module):
    def __init__(self, num_classes,pretrained_type):
        super(UNET, self).__init__()
        self.num_classes = num_classes
        if pretrained_type == 'false':
            self.encoder = resnet50(pretrained = False)
        elif pretrained_type == 'dp' :
            self.encoder = resnet50(pretrained = False)
        elif pretrained_type == 'ddp':
            self.encoder = resnet50(pretrained = True)
            for param in self.encoder.parameters():
                param.requires_grad = False
        elif pretrained_type == 'finetune-np':
            self.encoder = resnet50(pretrained = False)
        elif pretrained_type == 'finetune-v1-ef':
            self.encoder = resnet50(pretrained = False)
        else:
            self.encoder = resnet50(pretrained = True)
        ...
        self.out_denoise = nn.Conv2d(64, 1, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        output_denoise = self.out_denoise(out)
        return torch.sigmoid(outputs), output_denoise
