...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):
        ...
        for ii in range(batch_size):
            ...
            this_classes = [x for x in this_classes if x != self.ignore_label]  # 去除忽略的类别
            this_classes = [x for x in this_classes if (this_y == x).nonzero().shape[0] > self.max_views]  # 去除类别像素小于100的类别
        ...
        n_view = self.max_samples // total_classes  # 平均每个类别所占有的样本
        ...
        X_ptr = 0
        for ii in range(batch_size):  # 遍历每张图像
            this_y_hat = y_hat[ii]
            this_y = y[ii]
            this_classes = classes[ii]

            for cls_id in this_classes:  # 遍历每个类别
                hard_indices = ((this_y_hat == cls_id) & (this_y != cls_id)).nonzero()  # 将pred和lb不一致的定义为hard case
                easy_indices = ((this_y_hat == cls_id) & (this_y == cls_id)).nonzero()  # 将pred和lb一致的定义为easy case
    ...
    def _contrastive(self, feats_, labels_):
        '''
        feats_: [total_classes, n_view, feat_dim]
        labels_: [total_classes]
        '''
    ...
    def forward(self, feats, labels=None, predict=None):
        labels = labels.unsqueeze(1).float().clone()  # [N, 1, H, W]
        labels = torch.nn.functional.interpolate(labels,
                                                 (feats.shape[2], feats.shape[3]), mode='nearest')
        labels = labels.squeeze(1).long()  # [N, H, W]
        assert labels.shape[-1] == feats.shape[-1], '{} {}'.format(labels.shape, feats.shape)

        batch_size = feats.shape[0]

        labels = labels.contiguous().view(batch_size, -1)  # [N, HxW]
        predict = predict.contiguous().view(batch_size, -1)  # [N, HxW]
        feats = feats.permute(0, 2, 3, 1)  # [N, H, W, dim]
        feats = feats.contiguous().view(feats.shape[0], -1, feats.shape[-1])  # [N, HxW, dim]
